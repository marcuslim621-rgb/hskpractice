...
function startTileRecognition(pool,label){
  ...
  show('recognition');renderTileRecognition();
}
...
function exitTileRecognition(){saveTileRecognition();tileRound=null;show('home');}
function renderTileRecognition(){
  ...
  screen.querySelectorAll('[data-hand-index]').forEach(button=>button.onclick=()=>{
    ...
    if(picked!==question.w){r.missed=true;screen.querySelector('.rt-feedback').textContent=picked[0]+' is '+pin(picked[1])+' ('+picked[2]+'). Try another tile.';return;}
    const source=button.getBoundingClientRect();r.answered=true;r.matched.push(picked);r.results.push({c:wordKey(picked),ok:!r.missed});renderTileRecognition();animateTileToPile(source,r.matched.length-1,picked[0]);
  });
}
function animateTileToPile(source,index,text){
  ...
  const animation=flying.animate([{transform:`scale(${source.width/38},${source.height/50})`},{transform:`translate(${dx*.45}px,${dy*.7-25}px) scale(1.15) rotate(-9deg)`,offset:.5},{transform:`translate(${dx}px,${dy}px) rotate(-4deg)`}],{duration:620,easing:'cubic-bezier(.22,.7,.28,1)',fill:'forwards'});
  animation.finished.then(()=>{target.style.visibility='';flying.remove();}).catch(()=>flying.remove());
}
