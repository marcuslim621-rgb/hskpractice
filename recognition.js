...
function renderTileRecognition(phase='question'){
  ...
  screen.innerHTML=`<div class="rt-game${done?' rt-done':''}${spent?' rt-spent':''}"><header class="rt-head"><button class="rt-exit" type="button">← Leave hand</button><span>Hanzi Daily · Recognition</span></header><div class="rt-table"><h1>${done?'Nicely played.':question.kind==='pinyin'?'Match the pinyin':'Match the English meaning'}</h1>${statusBar}${done?'':tileMeld()}<div class="rt-center">${centre}</div>${done?tileMeld():''}<div class="rt-next-wrap">${action}</div><div class="rt-hand" aria-label="Character tiles">${r.rack.map((e,i)=>{const inUse=r.staged.includes(e);return `<button class="rt-tile${inUse?' rt-inuse':''}" type="button" data-rack-index="${i}" aria-label="Choose ${esc(e.ch)}"${done||r.answered||inUse?' disabled':''}>${esc(e.ch)}</button>`}).join('')}</div><p class="rt-foot">${r.questions.length} words · mixed pinyin & English</p></div></div>`;
